...
### 10. Schema 与真实数据库漂移后，后台接口直接读不存在的列
**错误**: Admin 用户详情页 / 帖子管理接口直接读取 `posts.status`
**表现**: `/api/admin/users/:id`、`/api/admin/posts` 在某些环境返回 500
**原因**:
- TypeScript schema 中定义了 `posts.status`
- 真实数据库中该列并不存在
- API 代码在查询、过滤、更新时都把它当作真实列使用，导致 SQL 执行失败

**教训**:
- 对历史库或多环境库，不能默认 schema.ts 与线上数据库完全一致
- 后台管理接口接触“状态筛选 / 状态操作”时，要先确认列已经通过 migration 正式落库
- 如果系统处于迁移中，应优先做运行时兼容或显式报 400，而不是继续让接口抛 500

### 11. 表名迁移未收敛时，API 只支持单一路径
**错误**: Admin 详情页和统计接口只查询 `friends` 或只查询 `friendships`
**表现**: 某些环境可用，某些环境直接 500
**原因**:
- 文档、schema、真实数据库在好友表名上存在双轨：`friends` 与 `friendships`
- 接口代码没有兼容层，默认只认其中一种
- 结果是只要部署环境与本地假设不同，就会立即炸

**教训**:
- 表名或字段名迁移不是“改 schema.ts 就完成”，必须同步 API、migration、文档和真实库
- 在结构未完全收敛前，后台接口应优先做双读兼容，确保管理能力不断
- 差异报告应明确区分“代码已兼容”和“数据库已统一”，避免误判修复状态
