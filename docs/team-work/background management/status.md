...
- 已定位后台 500 的一项根因：数据库连接池重复初始化导致 `Too many connections`，现已改为全局复用 pool。
- 后台关键页面已收敛鉴权头构造，没有 token 时不再主动发送空 Bearer 头，减少 `invalid token` 误报。
...
- 已进一步收敛后台请求头构造，避免发送 `Bearer null` 或 `Bearer undefined`。
- 已定位后台接口 `Too many connections` 的一项根因：数据库 pool 未做全局单例复用；现已修复，但仍需重启进程并观察是否恢复。
