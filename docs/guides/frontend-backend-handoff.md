...
| 变量 | 必填 | 说明 |
|------|------|------|
| `NEXT_PUBLIC_API_BASE_URL` | 联调后端时必填 | 浏览器请求用的 **API 根地址**（无尾斜杠）。常见两种用法：1) 直连后端根地址，如 `https://api.example.com`；2) 采用 Nginx 统一域名时设置为 `/api`（后续鉴权接口仍为 `/api/auth/*`）。空字符串时前端请求发往同源。 |
...
| `DATABASE_URL` | 认证启用时必填 | 外部数据库连接串（Postgres/MySQL，取决于后端 TypeORM 驱动）。 |
...
- 前端环境：确保 `NEXT_PUBLIC_API_BASE_URL` 指向你已部署的后端服务根地址（无尾斜杠），例如 `https://api.example.com`；若通过 Nginx 代理到后端，也可设置为 `/api`。
...
- 字段：
  ...
  - `created_at`（DATETIME/TIMESTAMP 类型，NOT NULL，默认当前时间）
